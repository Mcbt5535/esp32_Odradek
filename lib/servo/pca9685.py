...
class PCA9685:
    ...
    def set_pwm_freq(self, freq):
        ...
        # 计算预分频值
        prescale = int(25000000 / (4096 * freq) - 1) + 7  # +7是误差补偿
        print(f"PWM frequency: {freq} Hz, prescale: {prescale}")
        # 设置预分频
    ...
    def set_pwm(self, channel, on, off):
        ...
        if channel < 0 or channel > 15:
            ...
            return
        if on < 0 or on > 4095:
            print("Invalid on time")
            return
        if off < 0 or off > 4095:
            print("Invalid off time")
            return
        if on > off:
            print("Invalid (on > off)")
            return
    ...
    def set_servo_angle(self, channel, angle):
        ...
        pwm = int(((0.5 + (2 / 180) * angle) * 4096) / 20)
